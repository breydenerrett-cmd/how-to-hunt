...
func _ready() -> void:
 var sun=DirectionalLight3D.new();add_child(sun);sun.rotation_degrees=Vector3(-38,-28,0);sun.light_color=Color("ffe0ae");sun.light_energy=.78;sun.shadow_enabled=true;sun.directional_shadow_max_distance=105
 ...
 env.environment.background_mode=Environment.BG_SKY;env.environment.sky=sky;env.environment.ambient_light_source=Environment.AMBIENT_SOURCE_COLOR;env.environment.ambient_light_color=Color("aabdaf");env.environment.ambient_light_energy=.25;env.environment.ambient_light_sky_contribution=0
 env.environment.tonemap_mode=Environment.TONE_MAPPER_LINEAR;env.environment.fog_enabled=true;env.environment.fog_light_color=Color("a7b6a4");env.environment.fog_density=0.0025;env.environment.fog_light_energy=.4;env.environment.fog_sky_affect=.18
...
func terrain() -> void:
 ...
 var material=ShaderMaterial.new();material.shader=load("res://hunt/ground.gdshader");ground.material_override=material
...
func groves() -> void:
 ...
 var node=MultiMeshInstance3D.new();add_child(node);node.multimesh=mm;var m=V.material(Color.WHITE);m.vertex_color_use_as_albedo=true;node.material_override=m
func beam(parent:Node3D,a:Vector3,b:Vector3,r:float,color:Color) -> Node3D:
 var n=V.cylinder(parent,(a+b)*.5,r,a.distance_to(b),color,false,r*.8);n.quaternion=Quaternion(Vector3.UP,(b-a).normalized());return n
func lodge() -> void:
 ...
 for x in [-1,1]:
  var roof=V.box(root,Vector3(x*1.85,3.25,0),Vector3(3.95,.16,6.25),Color("394f4b"));roof.rotation.z=-x*.29
  for z in range(-3,4):
 ...
 for i in range(ids.size()):
  var p=Vector3(-2.5+i*1.25,1.08,-1.55)
  ...
  V.text3d(root,p+Vector3(0,.53,.08),C.ITEMS[ids[i]].name+"\n"+str(C.ITEMS[ids[i]].cost)+" credits",11,Color("f1ddb9"))
 for side in [-1,1]:
...
func exchange() -> void:
 ...
 var roof=V.box(root,Vector3(0,2.9,-.7),Vector3(5,.14,3.4),Color("ac875e"));roof.rotation.x=.08
...
func add_track(p:Vector3,heading:float,id:int) -> void:
 var node=Node3D.new();add_child(node);node.position=p+Vector3(0,.025,0);node.rotation.y=heading
 for x in [-.11,.11]:
  var hoof=V.sphere(node,Vector3(x,0,0),.075,Color("d1b671"));hoof.scale=Vector3(.65,.10,1.5)
 tracks.append({"p":p,"id":id,"time":time,"heading":heading});track_nodes.append(node)
 while tracks.size()>100:
  tracks.pop_front();track_nodes.pop_front().queue_free()
func clear_tracks() -> void:
 for node in track_nodes:node.queue_free()
 track_nodes.clear();tracks.clear()
...
func _exit_tree() -> void:
 Plants._meshes.clear();Plants._wind=null;Plants._bark=null
